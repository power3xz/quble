...
test("free하면 그 칸이 null로 비워지고(GC용) freeList에 인덱스가 쌓인다", () => {
  const pool: string[] = ["a", "b", "c"];
  const freeList: number[] = [];
  freeInPool(pool, freeList, 1);
  assert.equal((pool as (string | null)[])[1], null); // 빈 칸은 null(유닛 내부 세부)

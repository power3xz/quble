...
test("합성된 자식 이벤트는 fullname(자식 type-name 누적)으로 수집된다", () => {
  ...
  const fullnames = collectEventFullnames(module, 0);
  assert.deepEqual(fullnames, ["Toggle.TOGGLE"]);
});

test("루트 컴포넌트 자신의 이벤트는 prefix 없는 로컬명으로 수집된다", () => {
  // Toggle을 루트(comp 1)로 직접 보면 합성 경로가 없어 "TOGGLE".
  const fullnames = collectEventFullnames(module, 1);
  assert.deepEqual(fullnames, ["TOGGLE"]);
});

...
const detachOneBranch = (store, regions, branches, branchIndex) => {
...
const attachOneBranch = (store, regions, branches, anchor, branchIndex) => {
  const branch = branches[branchIndex];
...
const detachIf = (store, regions, branches, region) => {
...
// 균일 처리된다(분기 없음).
const attachIf = (store, regions, branches, region) => {
  attachOneBranch(
    store,
    regions,
    branches,
...
export const appendIfRegion = (regions, branches, condLeafIndex) => {
...
export const activateIf = (store, regions, branches, regionIndex, shownIndex) => {
  ...
  const nextBranch = branches[region.branchIndices[shownIndex]];
  if (!nextBranch.built) {
...
const detachFor = (store, regions, branches, region) => {
...
const attachFor = (store, regions, branches, region) => {
  for (const branchIndex of region.branchIndices) {
...
export const appendBranchOfForRegion = (regions, branches, regionIndex) => {
...
export const attachForIteration = (store, regions, branches, region, branchIndex) => {
  const branch = branches[branchIndex];
  const slot = region.branchIndices.indexOf(branchIndex);
  const prev = slot > 0 ? branches[region.branchIndices[slot - 1]] : null;
  const after =
...
export const truncateFor = (store, regions, branches, region, count) => {

...
> 수단이다. (런타임에 모듈을 따로 전송/연결하는 동적 링크는 미결, §5.3.)
...
눌렀을 때 핸들러는 하나인데, 어느 항목인지 어떻게 아나? 풀네임에 인덱스를 박을 수는 없다
(인덱스마다 핸들러를 따로 만들 수 없다). 그래서 "배열의 어느 요소인가"는 핸들러 이름이 아니라
**별도 식별 슬롯**이 답한다.

같은 모양의 문제가 셋이고 전부 이 한 슬롯으로 수렴한다.

- `@for`로 펼쳐진 배열 요소들
- 별칭 없이 중복된 형제 컴포넌트
- (그 외 같은 풀네임을 공유하는 모든 것)

이 슬롯의 이름/구조는 미결(§5.1). 컨텍스트 메타데이터(`context`)와는 성격이 달라 섞지 않는다.
...
핸들러가 같은 풀네임의 배열 요소를 구분할 식별 정보(§3.3). 컨텍스트 메타데이터는 `context`로
분리/결정됐고(§2.5), 이 슬롯은 이름/구조가 미결이다. 담아야 할 둘:

- **동적 리스트 항목** - 인덱스, 그리고/또는 안정적 key (리스트 재정렬 시 흔들리지 않도록)
- **정적 중복 형제** - 같은 풀네임을 공유하는 것 중 어느 것인지

이 슬롯의 이름과 구조를 결정해야 한다(§2.5 핸들러 시그니처의 빈 슬롯 자리).

...
type TEventHandler<TStore> = (
  ...
  params: {
    ...
    $0, $1, ...: number,  // @for 회차 인덱스(바깥 $0, 안쪽 $1)
    get: () => TStore,
...
**항목 식별자(key)는 두지 않는다.** 인덱스가 요소를 가리키는 주소가 아니라 요소가 가진 칸에
담긴 값이라, 리스트가 바뀌어도 항목 자신의 leafIndex는 그대로 산다 - 정체를 따로 붙일 이름이
필요 없다. 이유는 DECISIONS.md "배열 항목 식별자(key) - 도입 안 함".

...
이벤트별 플래그로 손으로 다는 건 핸들러 문법의 "도구가 떠받치는" 방향과 반대고 실수하기 쉬워
...
한 페이지가 컴포넌트 여러 개를 쓰고, 그중 상당수는 다른 페이지에서 이미 받아 뒀다. 지금은
그걸 모르고 `GET /component/<name>.qubb`로 매번 통째 받는다. 클라가 가진 목록을 서버가 알면
없는 것만 보낼 수 있다.

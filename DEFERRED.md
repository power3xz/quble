...
미룬 결정들. 지금 정할 근거가 부족하거나, 뒤에 올 작업이 판단 재료를 주는 것들이다.
정해서 처리하면 이 문서에서 지운다 - 여기 남아 있다는 건 아직 안 정했다는 뜻이다.

에디터 지원을 wasm으로 옮기는 작업(2026-08-03)에서 나왔다.
...
- **playground가 `quble-web`을 선언 안 함** - `../web/runtime.ts`를 상대경로로 쓰면서
  `dependencies`에 없다. `quble-wasm-compiler`는 이번에 패키지 import로 바꿨으나 이쪽은 기존
  방식 그대로 뒀다(이번 작업 범위 밖).
- **`compile_src`/`compile_file`의 접미사** - dts 쪽은 `_src`를 떼고 `_file`을 `_from_path`로
  바꿨는데(같은 이유: `_src`가 안 읽힘) 컴파일 쪽은 그대로다. 함께 갈지 안 정했다.
- **브랜치 머지** - `dts-array-ops`에 9커밋. main에 `--no-ff`로 넣을 시점이다.

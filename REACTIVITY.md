...
Quble의 반응성/핸들러 모델. **leafIndex**와 **fullname**으로 꿴 결론이다.
...
노드 이동은 하지 않는다 - 회차는 자기 자리에 있고 그 자리가 보는 값만 바뀐다. 그래서
재정렬(swap/move)도 회차 DOM을 옮기지 않고 두 요소의 칸 값을 서로 `set`하는 일로 끝난다.
요소가 배열을 품으면 그 두 배열끼리 다시 같은 규칙을 적용한다 - 길이가 다르면 안쪽 회차가
그만큼 생기거나 없어진다.
항목 식별자(key)를 두지 않는 이유는 DECISIONS.md "배열 항목 식별자(key) - 도입 안 함".
...
배열 요소 식별 슬롯(이름 미정)이 식별해야 할 "인스턴스 구분"이

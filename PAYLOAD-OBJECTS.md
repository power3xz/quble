# 이벤트 payload/context에 객체 전달 (아이디어 단계)

> 상태: 설계 탐색 + POC/벤치 검증 완료, proto 미구현. 구현 시 세부(인코딩·명령
> 이름 등)는 바뀔 수 있다. 이 문서는 "어떻게 풀지"의 방향과 그 근거, 그리고 기각한
> 대안을 남긴다. 확정된 바이트코드 계약은 구현 시 proto/BYTECODE.md에 들어간다.

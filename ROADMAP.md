...
확정 설계는 DESIGN.md, 바이트코드 명세는 core/BYTECODE.md, 보류 아이디어는 IDEAS.md.
...
- [ ] 컴포넌트 스타일링 - 외부 CSS를 어떻게 표현/격리할지(DESIGN.md 부록 B가 `import styles
  ...
  - [x] 정적 class 배열 - `class={["a", "b"]}`를 컴파일타임에 공백으로 이어 정적 값으로.
    바이트코드는 `class="a b"`와 같다(런타임이 배열을 모른다).
  - [ ] 격리, 동적 class 배열(요소에 변수 섞기 - 런타임 합치기가 필요). 방법 미정.

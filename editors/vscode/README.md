...
주소기라 스칼라는 leafIndex(`TLeafIndex<T>`)로 `get`/`set`하고, 객체/배열은 노드다. 객체는
필드로(`set(props.ghost.style, ...)`), 배열은 인덱스로 내려간다(`set(props.items[2].title, ...)`,
`props.items.length`). 통째로 갈아끼우려면 `setObject(props.ghost, {...})`(안 준 필드는
`undefined`) / `setArray(props.items, [...])`이고, 요소 추가/제거는 `push`/`removeAt`이다.
대상 종류가 안 맞으면 타입에서 걸린다(배열 자리에 leaf를 넘기는 식).

...
// 루트 prop leafIndex는 선언 순서(plant 순서)와 같다 - for_count는 n=0, for_if_count는 n=0,flag=1.
const instantiate = (name: string, values: Record<string, unknown>) => {
...
test("회차 제거 후 flag 토글: 잔류 구독 없이 남은 회차만 swap된다", () => {
  const { store, host } = instantiate("for_if_count", { n: 3, flag: true });
  assert.deepEqual(paragraphTexts(host), ["on", "on", "on"], "초기 n=3 모두 on");
  store.set(0, 1); // n: 회차 2개 제거 -> 그 @if 조건 구독도 해제되어야
...
test("회차 재증가로 칸 재사용 후 flag 토글: 재사용 region도 정상 swap", () => {
  const { store, host } = instantiate("for_if_count", { n: 3, flag: true });
  store.set(0, 1); // n: 회차 2개 제거(구독 해제 + 칸 반납)
  store.set(0, 3); // n: 반납된 칸 재사용해 회차 2개 재생성
  store.set(1, false); // flag
